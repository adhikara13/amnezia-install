...
select_client_ip() {
	...
	octet=2
	while grep AllowedIPs /etc/amnezia/amneziawg/wg0.conf | cut -d "." -f 4 | cut -d "/" -f 1 | grep -q "$octet"; do
		(( octet++ ))
	done
...
new_client_setup() {
	...
	if [[ "$specify_ip" =~ ^[yY]$ ]]; then
		...
		until [[ $client_ip =~ ^10\.7\.0\.([2-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-4])$ ]] \
			&& ! grep AllowedIPs /etc/amnezia/amneziawg/wg0.conf | cut -d "." -f 4 | cut -d "/" -f 1 | grep -q "$octet"; do
...
PresharedKey = $psk
AllowedIPs = 10.7.0.$octet/32$(grep -q 'fddd:2c4:2c4:2c4::1' /etc/amnezia/amneziawg/wg0.conf && echo ", fddd:2c4:2c4:2c4::$octet/128")
# END_PEER $client
EOF
...
[Interface]
Address = 10.7.0.$octet/24$(grep -q 'fddd:2c4:2c4:2c4::1' /etc/amnezia/amneziawg/wg0.conf && echo ", fddd:2c4:2c4:2c4::$octet/64")
DNS = $dns
PrivateKey = $key

[Peer]
PublicKey = $(grep PrivateKey /etc/amnezia/amneziawg/wg0.conf | cut -d " " -f 3 | awg pubkey)
PresharedKey = $psk
AllowedIPs = 0.0.0.0/0, ::/0
Endpoint = $(grep '^# ENDPOINT' /etc/amnezia/amneziawg/wg0.conf | cut -d " " -f 3):$(grep ListenPort /etc/amnezia/amneziawg/wg0.conf | cut -d " " -f 3)
...
if [[ ! -e /etc/amnezia/amneziawg/wg0.conf ]]; then
	...
	if [[ "$os" == "ubuntu" ]]; then
	    ...
	    (
	        set -x
	        apt-get -yqq update || apt-get -yqq update
	        apt-get -yqq install software-properties-common python3-launchpadlib gnupg2 linux-headers-$(uname -r) >/dev/null
	        add-apt-repository -y ppa:amnezia/ppa
	        apt-get -yqq update
	        apt-get -yqq install amneziawg qrencode $firewall >/dev/null
	    ) || exiterr2
	    ...
	    (
	        ...
	        dnf -y -q copr enable amneziavpn/amneziawg
	        dnf -y -q install amneziawg-dkms amneziawg-tools qrencode $firewall >/dev/null 2>&1
	    ) || exiterr3
		mkdir -p /etc/amnezia/amneziawg/
	elif [[ "$os" == "centos" && "$os_version" -eq 8 ]]; then
	    (
	        set -x
	        dnf -y -q install epel-release elrepo-release >/dev/null
	        dnf -y -q --nobest install kmod-wireguard >/dev/null 2>&1
	        dnf -y -q copr enable amneziavpn/amneziawg
	        dnf -y -q install amneziawg-dkms amneziawg-tools qrencode $firewall >/dev/null 2>&1
	    ) || exiterr3
		mkdir -p /etc/amnezia/amneziawg/
	    ...
	    (
	        ...
	        yum -y -q copr enable amneziavpn/amneziawg
	        yum -y -q install amneziawg-dkms amneziawg-tools qrencode $firewall >/dev/null 2>&1
	    ) || exiterr3
		mkdir -p /etc/amnezia/amneziawg/
	elif [[ "$os" == "fedora" ]]; then
	    (
	        ...
	        dnf -y install amneziawg-dkms amneziawg-tools qrencode $firewall >/dev/null
	    ) || exiterr "'dnf install' failed."
		mkdir -p /etc/amnezia/amneziawg/
	elif [[ "$os" == "openSUSE" ]]; then
	    (
	        set -x
	        zypper ar -f https://download.opensuse.org/repositories/home:Amnezia/openSUSE_Tumbleweed/home:Amnezia.repo
	        zypper refresh
	        zypper -n install amneziawg qrencode $firewall >/dev/null
	    ) || exiterr4
		mkdir -p /etc/amnezia/amneziawg/
	fi

	[ ! -d /etc/amnezia/amneziawg/ ] && exiterr2
...
WantedBy=multi-user.target" >> /etc/systemd/system/wg-iptables.service
	...
	new_client_setup
	# Enable and start the awg-quick service
	(
		set -x
		systemctl enable --now awg-quick@wg0.service >/dev/null 2>&1
	)
	echo
	...
	case "$option" in
		1)
			...
			set_client_name
			while [[ -z "$client" ]] || grep -q "^# BEGIN_PEER $client$" /etc/amnezia/amneziawg/wg0.conf; do
		...
		2)
			...
			num_of_clients=$(grep -c '^# BEGIN_PEER' /etc/amnezia/amneziawg/wg0.conf)
			...
			echo
			grep '^# BEGIN_PEER' /etc/amnezia/amneziawg/wg0.conf | cut -d ' ' -f 3 | nl -s ') '
		...
		3)
			...
			client=$(grep '^# BEGIN_PEER' /etc/amnezia/amneziawg/wg0.conf | cut -d ' ' -f 3 | sed -n "$client_num"p)
			echo
			...
			if [[ "$remove" =~ ^[yY]$ ]]; then
				...
				awg set wg0 peer "$(sed -n "/^# BEGIN_PEER $client$/,\$p" /etc/amnezia/amneziawg/wg0.conf | grep -m 1 PublicKey | cut -d " " -f 3)" remove
		...
		5)
			...
			if [[ "$remove" =~ ^[yY]$ ]]; then
				...
				port=$(grep '^ListenPort' /etc/amnezia/amneziawg/wg0.conf | cut -d " " -f 3)
				if systemctl is-active --quiet firewalld.service; then
					...
					if grep -qs 'fddd:2c4:2c4:2c4::1/64' /etc/amnezia/amneziawg/wg0.conf; then
				...
				systemctl disable --now awg-quick@wg0.service
